...
class Expression{
	...
	String PrintExpression(){
		...
		String merge_exp = null;
		String part_exp = null;
		...
		merge_exp = exp.toString();
		
		for(Term t: terms){
			part_exp = t.stringFormat();
			//System.out.println(part_exp+"!");
			for(int index=0;index < part_exp.length();){
				if(Character.isLetter(part_exp.charAt(index)) == true){
					
					//System.out.println(part_exp.substring(index));
					//merge_exp = Merge(merge_exp, part_exp.substring(index));
					//System.out.println(merge_exp);
					break;
...
public class caculate {
	...
	public static void main(String[] args) {//main method
		...
		String s = Expression.Merge("2*x*x*y*z+4*x*z+2*y*z+z*x+y*z+x*x*y*z");
		...
		if(matcher.matches()){
			...
			if(command_re == "1"){
				...
				if(!exp1.PrintExpression().equals(express1)){
					System.out.println(exp1.PrintExpression());
				}else{
